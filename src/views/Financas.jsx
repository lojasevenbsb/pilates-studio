...
import { brl, primeiroNome } from "../utils/format.js";
      ...
      {lista.length === 0
        ...
        : lista.map(a => {
            ...
            return (
              <div
                key={a._id}
                onClick={() => router.push(`/alunos/${a._id}?tab=mens`)}
                style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:m?.pago?"#f0faf3":"#fff", border:`1.5px solid ${m?.pago?"#a8c5ab":"#e8e0d0"}`, borderRadius:12, padding:"14px 16px", marginBottom:8, transition:"all .15s", cursor:"pointer" }}
              >
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <div style={{ width:30, height:30, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", background:m?.pago?"#3b5c3e":"#f4efe5", fontSize:12, fontWeight:700, color:m?.pago?"#fff":"#8c8c8c", flexShrink:0 }}>{numParcela}</div>
                  <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  {m?.pago ? (
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); openPago(a._id, mes); }}
